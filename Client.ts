...
export default class InfernoClient extends Client {
  ...
  public async init() {
    ...
    this.on('modalSubmit', async (modal) => {
      ...
      const answer2 = modal.getTextInputValue('Reason' + modal.user.id);
         ...
         logs.send( { embeds: [embed] });
          ...
          connection.query(
            'UPDATE applications SET status = "accepted", log = ? WHERE id = ?',
            [modal.user.id, answer1],
            async function(error: any, results: any, fields: any) {
              ...
              // Send a DM to the user who made the application
              const discordID = await getApplicationStatus(answer1);
              if (discordID) {
                const user = await modal.client.users.fetch(discordID);
    ...
    ping.send('<@&963903090057154570>')
     ...
     ping.send('<@&963903090057154570>')
      ...
      .catch(error => {
      console.error(error);
      modal.reply('An error occurred while checking your application status. Please try again later, or you can check at https://vantage.kieranbot.info/applications');
      });
    }
  });
  ...
  async function checkApplicationStatus(answer1: any) {
    ...
    const embed = new MessageEmbed()
      .setColor(0xED4245)
      .setTitle('Error')
      .setDescription('An error occurred while checking your application status. Please try again later, or you can check at https://vantage.kieranbot.info/applications');
  ...
  function getApplicationStatus(applicationID: any) {
    return new Promise((resolve, reject) => {
      // Query the MySQL database to get the application status and Discord ID using the given application ID
      connection.query(
        'SELECT status, discord FROM applications WHERE id = ?',
        [applicationID],
        function (error: any, results: any[], fields: any) {
          ...
          if (results.length === 0) {
            const embed = new MessageEmbed()
              .setColor(0xED4245)
              .setTitle('Error')
              .setDescription(`Application ${applicationID} not found`);
  
            return reject(embed);
          }
  
          const { status, discord } = results[0];
          resolve({ status, discord });
        }
      );
    });
  }
